    ...
    <div className='bg-half-transparent w-screen fixed nav-item top-0 right-0'>
      <div className="float-right h-screen dark:text-gray-200 bg-white dark:[#484852] w-400">
        <div className="flex justify-between items-center p-4 ml-4">
          ...
          <button type='button' onClick={() => setThemeSettings(false)} style={{color: 'rgb(153, 171, 100)', borderRadius: '50%'}} className='text-2xl p-3 hover:drop-shadow-xl hover:bg-light-gray'>
            <MdOutlineCancel></MdOutlineCancel>
          </button>
        </div>
        <div className="flex-col border-t-1 border-color p-4 ml-">
              ...
              <TooltipComponent key={index} content={item.name} position='TopCenter'>
                <div className="relative mt-2 cursor-pointer flex gap-5 items-center">
                  <button type='button' className='h-10 w-10 rounded-full cursor-pointer' style={{backgroundColor: item.color}} onClick={()=> setColor(item.color)}>
                    <BsCheck className={`ml-2 text-2xl text-white ${(item.color === currentColor)?('block'):('hidden')}`}></BsCheck>
                  </button>
                </div>
              </TooltipComponent>
            ))}
